...
> **Audience:** end users on macOS. The current `.mcpb` ships darwin prebuilds only; cross-platform builds are planned.
...
2. **Node.js ≥ 20** available at `/usr/local/bin/node` or `/opt/homebrew/bin/node`. The simplest install is `brew install node`. Verify:
   ```bash
   ls /usr/local/bin/node /opt/homebrew/bin/node 2>/dev/null
   node --version
   ```
   Why this matters: the `.mcpb` launches its child process via `/usr/bin/env node`, which searches Claude Desktop's inherited `PATH`. Homebrew installs node at one of the two paths above, both of which are on the default `PATH`. Node managed only by `nvm` is typically *not* on Claude Desktop's `PATH` and won't work without a symlink.
...
- **[`image-gen.mcpb`](https://github.com/Cleveland-RD/claude-image-gen/releases/latest/download/image-gen.mcpb)** (latest release)
...
5. Select the downloaded `image-gen.mcpb`.
...
Each field is individually optional — fill the one(s) you actually plan to use. Both are marked `sensitive: true` in the manifest, so values are stored in **macOS Keychain**, not in plaintext on disk.
...
Fully quit Claude Desktop with **Cmd-Q** (closing the window isn't enough) and reopen. This makes Claude Desktop re-spawn the MCP child process with the API keys you just saved.
...
- A PNG lands at `~/Pictures/image-gen/<slug>-<hash>.png`.
...
1. **`/usr/bin/env node` can't find node.** The `.mcpb` resolves Node from Claude Desktop's inherited `PATH`. If you only have `nvm`-managed Node, it won't be there.
   ```bash
   # Are you set up?
   ls /usr/local/bin/node /opt/homebrew/bin/node /usr/bin/node 2>/dev/null
   ```
   If none of those exist, run `brew install node` (or symlink your nvm node into `/usr/local/bin`), then **fully quit Claude Desktop** (Cmd-Q — closing the window isn't enough) and reopen.
2. **A previous bad install is still loaded.** Settings → Extensions → remove `image-gen` → reinstall.

For both cases, the MCP's wrapper writes a diagnostic trail to `~/image-gen-mcp-debug.log`. If the file exists, open it — the boot line shows the Node version and `execPath` that actually ran. If it's missing, the wrapper never ran (most likely the PATH issue above).
...
The keys are read once, when the MCP launches. After clicking **Configure** and updating a key, the Keychain entry updates immediately but the *running* MCP process doesn't reload its env on the fly. **Fully quit Claude Desktop (Cmd-Q) and reopen** to pick up the new value. A new chat alone isn't sufficient — the MCP is spawned at app launch.
...
| Cached PNGs | `rm -rf ~/Pictures/image-gen` |
| Diagnostic log | `rm ~/image-gen-mcp-debug.log` |

Removing the `.mcpb` also drops the API keys from Keychain.
